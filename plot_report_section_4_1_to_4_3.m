...
% It uses final report titles, legends, and output filenames.
    ...
    legend(ax1, [hS0_1, hS1_1], ...
        {'BT under S0', 'BT under S1'}, ...
        'Location', 'eastoutside', ...
        'FontSize', 10, ...
        'Box', 'on');

    ax2 = nexttile;
    [hS0_2, hS1_2] = plot_topic_compare_panel(ax2, tt, Y0_all(:,q), Y1_all(:,q), ...
        'Q trajectories', colQ, deltaMain, colS0, zeroCol);
    legend(ax2, [hS0_2, hS1_2], ...
        {'Q under S0', 'Q under S1'}, ...
        'Location', 'eastoutside', ...
        'FontSize', 10, ...
        'Box', 'on');

    ax3 = nexttile;
    [hS0_3, hS1_3] = plot_topic_compare_panel(ax3, tt, Y0_all(:,om), Y1_all(:,om), ...
        'OM trajectories', colOM, deltaMain, colS0, zeroCol);
    legend(ax3, [hS0_3, hS1_3], ...
        {'OM under S0', 'OM under S1'}, ...
        'Location', 'eastoutside', ...
        'FontSize', 10, ...
        'Box', 'on');
    ...
    title(tl1, sprintf('S1 main quality shock (\\Delta Q = %.2f): all-agent mean trajectories compared with S0', deltaMain), ...
    ...
    title(tl2, sprintf('BT trajectories by role under S0 and S1 (\\Delta Q = %.2f)', deltaMain), ...
        'FontSize', 16, 'FontWeight', 'bold');
    ...
    title(tl3, sprintf('Role-specific mean trajectories across BT, Q, and OM under S0 and S1 (\\Delta Q = %.2f)', deltaMain), ...
        'FontSize', 16, 'FontWeight', 'bold');
...
function [hS0, hS1] = plot_topic_compare_panel(ax, tt, Y0_topic, Y1_topic, panelTitle, topicColor, deltaQ, colS0, zeroCol)
    ...
    hS1 = plot(ax, tt, Y1_topic, '-', ...
        'Color', topicColor, ...
        'LineWidth', 2.8, ...
        'DisplayName', sprintf('S1 quality shock (\\Delta Q = %.2f)', deltaQ));
